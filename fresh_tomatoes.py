...
tile_content = '''
<div class="col-md-4 tile text-center mx-auto"
        data-trailer-youtube-id="{trailer_youtube_id}" data-toggle="modal"
        data-target="#trailer">
    <img class="rounded-corners" src="{poster_image_url}" width="220"
        height="342">
    <h2>{title}</h2>
</div>
'''

# Creates the html content for movie and tv_show tiles.Extracts the youtube
# url and info required for display from each object in the passed list.
# Differentiates between movies and tv_shows. Processes TV-shows such that
# every show is only displayed once on the site, no matter how many episodes
# the list contains. Returns an array containing two strings: html for movies
# and tv-shows.
...
def create_movie_tiles_content(videos):
    ...
    for medium in videos:
        ...
        elif (isinstance(medium, media.TvShow) and
              medium.title not in found_shows):
...
# Creates a new html file, writes the static
# content, replaces keys with created
